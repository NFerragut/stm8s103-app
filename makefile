...
SERDIR = lib/serial
SERSRCS := $(shell ls $(SERDIR)/*.s)
SEROBJS = $(addsuffix .o,$(basename $(SERSRCS)))
...
libs : $(LIBDIR)/libapp0.sm8 $(LIBDIR)/libis0.sm8
...
$(SERDIR)/%.o : $(SERDIR)/%.s
	@echo $<:
	@cd $(SERDIR); $(AS) $(AFLAGS) $(notdir $<)

$(LISBDIR)/%.o : $(LIBDIR)/%.s
	@echo $<:
	@cd $(LIBDIR); $(AS) $(AFLAGS) $(notdir $<)

$(LIBDIR)/libis0.sm8 : $(SERSRCS) $(SEROBJS)
	@echo $@:
	@cp $(CXLIB)/libis0.sm8 $(LIBDIR)/libis0.sm8
	@clib -d $(LIBDIR)/libis0.sm8 getchr.o putchr.o
	@clib -r $(LIBDIR)/libis0.sm8 $(SEROBJS)
	@echo
...
$(OUTDIR)/$(APP).sm8 : $(LIBDIR)/libis0.sm8 $(LIBDIR)/libapp0.sm8 $(SOURCES) $(OBJECTS) $(OUTDIR)/$(APP).lkf
	@echo $@:
